...
export class MiDashBoardComponent {
  ...
  cards = this.breakpointObserver.observe(Breakpoints.Handset).pipe(
    map(({ matches }) => {
      if (matches) {
        return [
          { title: 'RPS', cols: 1, rows: 1, img: "ppt.png", texto: 'Elija su arma y juegue contra la computadora', ruta: '/rps' },
          { title: 'Aritmetica', cols: 1, rows: 1, img: "aritmetica.jpg", texto: 'Pruebe la destreza de su mente', ruta: '/Aritmetica' },
          { title: 'Adivine el numero', cols: 1, rows: 1, img: "adivina.jpg", texto: 'Pruebe su suerte', ruta: '/AdivinaNumero' },
